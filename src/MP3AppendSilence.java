...
public class MP3LengthExtender {
            ...
            for (File file : files) {
                if (file.isFile() && file.getName().toLowerCase().endsWith(".mp3")) {
                    try {
                        ...
                        double duration = scanner.hasNext() ? Double.parseDouble(scanner.next()) : 0;
                        scanner.close();
                        ...
                        // 合并原始音频和空音频
                        String mergeCommand = "ffmpeg -i \"" + file.getAbsolutePath() + "\" -i \"" + file.getParent() + File.separator + "silence.mp3\" -filter_complex '[0:a][1:a]concat=n=2:v=0:a=1[out]' -map \"[out]\" -c:a libmp3lame -q:a 2 -ar 44100 -ac 2 \"" + outputFilePath + "\"";
